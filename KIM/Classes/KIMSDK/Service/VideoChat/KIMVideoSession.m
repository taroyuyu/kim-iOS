...
typedef NS_ENUM(NSUInteger,KIMVideoSessionMode)
{
    KIMVideoSessionMode_Initiative,
    KIMVideoSessionMode_Passive,
};

@interface KIMVideoSession()<KIMVideoChatModuleSignalDelegate,RTCPeerConnectionDelegate>
@property(nonatomic,assign)uint64_t offerId;
@property(nonatomic,assign)KIMVideoSessionMode mode;
...
- (void)startSession
{
    ...
    [self setMode:KIMVideoSessionMode_Initiative];
    ...
    //3.3添加视频轨道
    self.localVideoSource = [self.peerConnectionFactory videoSource];
    __weak KIMVideoSession * weakSelf = self;
    
#if !TARGET_IPHONE_SIMULATOR
    if([self.delegate respondsToSelector:@selector(videoSessionDidCreateLocalVideoCapture:)]){
        RTCCameraVideoCapturer *capturer = [[RTCCameraVideoCapturer alloc] initWithDelegate:self.localVideoSource];
        if ([[NSThread currentThread] isEqual:[NSThread mainThread]]) {
                [weakSelf.delegate videoSessionDidCreateLocalVideoCapture:capturer];
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf.delegate videoSessionDidCreateLocalVideoCapture:capturer];
            });
    ...
    if (@available(iOS 10, *)) {
        if ([self.delegate respondsToSelector:@selector(videoSessionDidCreateLocalFileCapturer:)]) {
            RTCFileVideoCapturer *fileCapturer = [[RTCFileVideoCapturer alloc] initWithDelegate:self.localVideoSource];
            if ([[NSThread currentThread] isEqual:[NSThread mainThread]]) {
                    [weakSelf.delegate videoSessionDidCreateLocalFileCapturer:fileCapturer];
            }else{
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakSelf.delegate videoSessionDidCreateLocalFileCapturer:fileCapturer];
                });
...
#endif
    //3.3.4创建本地视频轨道轨道
    RTCVideoTrack * localVideoTrack = [self.peerConnectionFactory videoTrackWithSource:self.localVideoSource trackId:@"localVideoTrack"];
    [localMediaStream addVideoTrack:localVideoTrack];
    
    if ([self.delegate respondsToSelector:@selector(videoSessionDidReceiveLocalVideoTrack:)]) {
        if ([[NSThread currentThread] isMainThread]) {
            [self.delegate videoSessionDidReceiveLocalVideoTrack:localVideoTrack];
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate videoSessionDidReceiveLocalVideoTrack:localVideoTrack];
            });
...
{
    if (KIMVideoSessionMode_Initiative  == self.mode) {
        //接收到对方的应答,设置对方的会话描述符
...
-(void)waitSession
{
    ...
    [self setMode:KIMVideoSessionMode_Passive];
    //1.初始化peerConnection对象
    //1.1设置本次对等连接的相关约束:仅支持语音通话，不支持视频通话。开启安全实时传输
    RTCMediaConstraints* peerConnectionConstraints = [[RTCMediaConstraints alloc] initWithMandatoryConstraints:@{@"OfferToReceiveAudio":OfferToReceiveAudio,@"OfferToReceiveVideo":OfferToReceiveAudio} optionalConstraints:@{@"internalSctpDataChannels":@"true",@"DtlsSrtpKeyAgreement":@"true"}];
...
#if !TARGET_IPHONE_SIMULATOR
    if([self.delegate respondsToSelector:@selector(videoSessionDidCreateLocalVideoCapture:)]){
        RTCCameraVideoCapturer *capturer = [[RTCCameraVideoCapturer alloc] initWithDelegate:weakSelf.localVideoSource];
        if ([[NSThread currentThread]isMainThread]) {
            [self.delegate videoSessionDidCreateLocalVideoCapture:capturer];
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf.delegate videoSessionDidCreateLocalVideoCapture:capturer];
            });
    ...
    if (@available(iOS 10, *)) {
        if ([self.delegate respondsToSelector:@selector(videoSessionDidCreateLocalFileCapturer:)]) {
            RTCFileVideoCapturer *fileCapturer = [[RTCFileVideoCapturer alloc] initWithDelegate:self.localVideoSource];
            if ([[NSThread currentThread]isMainThread]) {
                [self.delegate videoSessionDidCreateLocalFileCapturer:fileCapturer];
            }else{
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakSelf.delegate videoSessionDidCreateLocalFileCapturer:fileCapturer];
                });
...
{
    if (KIMVideoSessionMode_Passive == self.mode) {
        __weak KIMVideoSession *weakSelf = self;
...
{
    //接收到来自对方的媒体流
    NSLog(@"接收到来自对方的媒体流，包含 %lu个视频轨道和%lu个音频轨道",(unsigned long)stream.videoTracks.count,(unsigned long)stream.audioTracks.count);
    if (stream.videoTracks.count && [self.delegate respondsToSelector:@selector(videoSessionDidReceiveRemoteVideoTrack:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            for (RTCVideoTrack * videoTrack in stream.videoTracks) {
                [self.delegate videoSessionDidReceiveRemoteVideoTrack:videoTrack];
            }
        });
    }
}
...
{
    NSLog(@"对方移除了%lu个视频轨道和%lu个音频轨道",(unsigned long)stream.videoTracks.count,(unsigned long)stream.audioTracks.count);
    if (stream.videoTracks.count && [self.delegate respondsToSelector:@selector(videoSessionDidRemovedRemoteVideoTrack:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            for (RTCVideoTrack * videoTrack in stream.videoTracks) {
                [self.delegate videoSessionDidRemovedRemoteVideoTrack:videoTrack];
            }
        });
    }
}
